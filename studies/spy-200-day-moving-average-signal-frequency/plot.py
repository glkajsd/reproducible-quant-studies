#!/usr/bin/env python3
"""Generate SVG charts from the signal-frequency backtest CSV outputs."""
...
def _read_equity() -> pd.DataFrame:
    path = OUTPUT_DIR / "spy-200dma-signal-frequency-equity.csv"
    if not path.exists():
...
def _read_summary() -> pd.DataFrame:
    path = OUTPUT_DIR / "spy-200dma-signal-frequency-summary.csv"
    if not path.exists():
        raise FileNotFoundError(f"Missing {path}; run backtest.py first")
...
def write_equity_chart(df: pd.DataFrame) -> None:
    index = _index(df)
    specs = [
        SeriesSpec("Daily", _variant_series(df, "Daily", "StrategyEquity"), COLORS["daily"], 2.3),
        SeriesSpec("Weekly", _variant_series(df, "Weekly", "StrategyEquity"), COLORS["weekly"], 2.2),
        SeriesSpec("Month-end", _variant_series(df, "Month-end", "StrategyEquity"), COLORS["month_end"], 2.2),
        SeriesSpec("SPY buy and hold", _variant_series(df, "Daily", "BenchmarkEquity"), COLORS["benchmark"], 1.8, "7 5"),
    ]
    ...
    body.append('<text x="78" y="484" class="subtitle">Log scale. All strategy variants use the same 200-trading-day SMA and 5 bps base costs.</text>')

    svg = _base_svg(
        "SPY 200DMA Signal Frequency",
        "Equity curve by signal-check frequency, $10,000 initial capital",
        "\n".join(body),
    )
    path = CHART_DIR / "spy-200dma-signal-frequency-equity-curve.svg"
...
def write_drawdown_chart(df: pd.DataFrame) -> None:
    index = _index(df)
    specs = [
        SeriesSpec("Daily", _variant_series(df, "Daily", "StrategyDrawdown") * 100, COLORS["daily"], 2.2),
        SeriesSpec("Weekly", _variant_series(df, "Weekly", "StrategyDrawdown") * 100, COLORS["weekly"], 2.1),
        SeriesSpec("Month-end", _variant_series(df, "Month-end", "StrategyDrawdown") * 100, COLORS["month_end"], 2.1),
        SeriesSpec("SPY buy and hold", _variant_series(df, "Daily", "BenchmarkDrawdown") * 100, COLORS["benchmark"], 1.8, "7 5"),
    ]
    ...
    svg = _base_svg(
        "SPY 200DMA Signal Frequency Drawdowns",
        "Peak-to-trough drawdown by signal-check frequency",
        "\n".join(body),
    )
    path = CHART_DIR / "spy-200dma-signal-frequency-drawdowns.svg"
    path.write_text(svg, encoding="utf-8")
    print(f"Wrote {path}")
...
def write_position_changes_chart(summary: pd.DataFrame) -> None:
    ...
    base["Position changes"] = base["Position changes"].astype(float)
    variants = ["Daily", "Weekly", "Month-end"]
    values = [float(base.loc[base["Strategy variant"] == variant, "Position changes"].iloc[0]) for variant in variants]
    ...
    for idx, (variant, value) in enumerate(zip(variants, values)):
        ...
        parts.append(
            f'<text x="{plot_x + width + 10:.1f}" y="{y + 29}" class="bar-label">{value:.0f}</text>'
        )
    ...
    svg = _base_svg(
        "SPY 200DMA Position Changes",
        "Turnover by signal-check frequency",
        "\n".join(parts),
    )
    path = CHART_DIR / "spy-200dma-signal-frequency-position-changes.svg"
    path.write_text(svg, encoding="utf-8")
    print(f"Wrote {path}")
