...
The strategy holds SPY when the previous adjusted close is above the previous
200-day simple moving average. Otherwise it holds cash. Returns are modeled
with a close-to-close approximation using adjusted close data from Yahoo
...
def prepare_signals(price_df: pd.DataFrame) -> pd.DataFrame:
    """Add SMA, signal, lagged position, and SPY returns."""
    df = price_df.copy()
    if "Date" not in df.columns:
    ...
    df["Signal"] = (
        (df["SMA200"].notna()) & (df["AdjustedClose"] > df["SMA200"])
    ).astype(int)
    ...
    # This is not a strict next-open or next-close executable order model.
...
def write_outputs(price_df: pd.DataFrame) -> None:
    ...
    summary_path = OUTPUT_DIR / "spy-200dma-summary.csv"
    equity_path = OUTPUT_DIR / "spy-200dma-equity.csv"
    trades_path = OUTPUT_DIR / "spy-200dma-trades.csv"
