#!/usr/bin/env python3
"""Generate SVG charts from the SPY 200DMA backtest CSV outputs.
...
def _read_equity() -> pd.DataFrame:
    path = OUTPUT_DIR / "spy-200dma-equity.csv"
    if not path.exists():
...
def write_equity_chart(df: pd.DataFrame) -> None:
    specs = [
        SeriesSpec("200DMA strategy, 5 bps costs", df["StrategyEquity"], COLORS["strategy"], 2.4),
        SeriesSpec("SPY buy and hold", df["BenchmarkEquity"], COLORS["benchmark"], 1.9, "7 5"),
    ]
    ...
    body = [_axis_svg(df.index, [(tick, _money_label(tick)) for tick in ticks], minimum, maximum, True)]
    ...
    body.append('<text x="78" y="484" class="subtitle">Log scale. Strategy uses prior close vs prior SMA200; cash return is 0%.</text>')
    ...
    path = CHART_DIR / "spy-200dma-equity-curve.svg"
...
def write_drawdown_chart(df: pd.DataFrame) -> None:
    specs = [
        SeriesSpec("Strategy drawdown", df["StrategyDrawdown"] * 100, COLORS["drawdown"], 2.2),
        SeriesSpec("Benchmark drawdown", df["BenchmarkDrawdown"] * 100, COLORS["benchmark_drawdown"], 1.8, "7 5"),
    ]
    ...
    svg = _base_svg(
        "SPY 200DMA Drawdowns",
        "Peak-to-trough drawdown, strategy versus buy and hold",
        "\n".join(body),
    )
    path = CHART_DIR / "spy-200dma-drawdowns.svg"
    path.write_text(svg, encoding="utf-8")
    print(f"Wrote {path}")


def _risk_on_rectangles(df: pd.DataFrame, min_value: float, max_value: float) -> str:
    parts = []
    in_segment = False
    start_date = None
    previous_date = None
...
def write_price_chart(df: pd.DataFrame) -> None:
    specs = [
        ...
        SeriesSpec("SMA200", df["SMA200"], COLORS["sma"], 1.8),
    ]
    ...
    body.append(_legend(specs, y=438))
    body.append('<text x="470" y="442" class="legend">Shaded area: strategy risk-on position</text>')
    for spec in specs:
        body.append(
    ...
    svg = _base_svg(
        "SPY Adjusted Close and SMA200",
        "Signal is based on the prior close and prior SMA200",
        "\n".join(body),
    )
    path = CHART_DIR / "spy-200dma-price-sma.svg"
    path.write_text(svg, encoding="utf-8")
    print(f"Wrote {path}")
