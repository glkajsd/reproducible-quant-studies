#!/usr/bin/env python3
"""Generate SVG charts from the ETF momentum rotation CSV outputs.

This script reads files produced by backtest.py. It does not recompute
momentum, weights, returns, costs, equity, or drawdowns.
"""
...
def _read_equity() -> pd.DataFrame:
    path = OUTPUT_DIR / "spy-qqq-sector-momentum-rotation-equity.csv"
    if not path.exists():
...
def _read_summary() -> pd.DataFrame:
    path = OUTPUT_DIR / "spy-qqq-sector-momentum-rotation-summary.csv"
    if not path.exists():
        raise FileNotFoundError(f"Missing {path}; run backtest.py first")
...
def write_equity_chart(df: pd.DataFrame) -> None:
    index = _index(df)
    specs = [
        SeriesSpec(VARIANT_TOP_1, _variant_series(df, VARIANT_TOP_1, "Strategy equity"), COLORS["top_1"], 2.4),
        SeriesSpec(VARIANT_TOP_3, _variant_series(df, VARIANT_TOP_3, "Strategy equity"), COLORS["top_3"], 2.3),
        SeriesSpec("SPY buy and hold", _variant_series(df, VARIANT_TOP_1, "Benchmark equity"), COLORS["benchmark"], 1.8, "7 5"),
    ]
    minimum = min(spec.values.min() for spec in specs) * 0.92
    maximum = max(spec.values.max() for spec in specs) * 1.08
    ticks = [
        tick
        for tick in [5_000, 10_000, 20_000, 50_000, 100_000, 200_000, 500_000, 1_000_000]
    ...
    body.append('<text x="78" y="484" class="subtitle">Log scale. Rotation variants use 5 bps base costs and one-day-lagged month-end weights.</text>')

    svg = _base_svg(
        "SPY QQQ Sector Momentum Rotation",
        "Equity curve, $10,000 initial capital",
        "\n".join(body),
    )
    path = CHART_DIR / "spy-qqq-sector-momentum-rotation-equity-curve.svg"
...
def write_drawdown_chart(df: pd.DataFrame) -> None:
    index = _index(df)
    specs = [
        SeriesSpec(VARIANT_TOP_1, _variant_series(df, VARIANT_TOP_1, "Strategy drawdown") * 100, COLORS["top_1"], 2.3),
        SeriesSpec(VARIANT_TOP_3, _variant_series(df, VARIANT_TOP_3, "Strategy drawdown") * 100, COLORS["top_3"], 2.2),
        SeriesSpec("SPY buy and hold", _variant_series(df, VARIANT_TOP_1, "Benchmark drawdown") * 100, COLORS["benchmark"], 1.8, "7 5"),
    ]
    ...
    svg = _base_svg(
        "SPY QQQ Sector Rotation Drawdowns",
        "Peak-to-trough drawdown by strategy variant",
        "\n".join(body),
    )
    path = CHART_DIR / "spy-qqq-sector-momentum-rotation-drawdowns.svg"
    path.write_text(svg, encoding="utf-8")
    print(f"Wrote {path}")


def write_turnover_chart(summary: pd.DataFrame) -> None:
    base = summary[summary["Cost bps"] == BASE_COST_BPS].copy()
    variants = [VARIANT_TOP_1, VARIANT_TOP_3]
    turnover_values = [
        float(base.loc[base["Strategy variant"] == variant, "Total turnover"].iloc[0])
        for variant in variants
    ]
    rebalance_values = [
        float(base.loc[base["Strategy variant"] == variant, "Rebalance count"].iloc[0])
        for variant in variants
    ]
    max_turnover = max(turnover_values) if turnover_values else 1.0
    ...
    for i in range(5):
        value = max_turnover * i / 4
        x = plot_x + value / max_turnover * plot_w if max_turnover else plot_x
        parts.append(
            f'<line x1="{x:.1f}" x2="{x:.1f}" y1="{plot_y}" y2="{plot_y + plot_h}" stroke="{COLORS["grid"]}" />'
        )
        parts.append(
    ...
    for idx, variant in enumerate(variants):
        ...
        turnover = turnover_values[idx]
        rebalances = rebalance_values[idx]
        bar_w = 0 if max_turnover == 0 else turnover / max_turnover * plot_w
        color = COLORS["top_1"] if variant == VARIANT_TOP_1 else COLORS["top_3"]
        parts.append(
            f'<text x="{plot_x - 18}" y="{y + 29}" text-anchor="end" class="bar-label">{html.escape(variant)}</text>'
        )
        parts.append(
            f'<rect x="{plot_x}" y="{y}" width="{bar_w:.1f}" height="{bar_h}" fill="{color}" opacity="0.88" />'
        )
        parts.append(
            f'<text x="{label_x}" y="{y + 18}" class="bar-label">{turnover:.1f} turnover</text>'
        )
        parts.append(
            f'<text x="{label_x}" y="{y + 38}" class="subtitle">{rebalances:.0f} rebalances</text>'
        )

    parts.append('<text x="78" y="438" class="subtitle">Turnover is the sum of absolute portfolio-weight changes. Full 100% ETF-to-ETF switches count as 2.0.</text>')
    svg = _base_svg(
        "Rotation Turnover",
        "Base-case total turnover and rebalance count, 5 bps cost scenario",
        "\n".join(parts),
    )
    path = CHART_DIR / "spy-qqq-sector-momentum-rotation-turnover.svg"
    path.write_text(svg, encoding="utf-8")
    print(f"Wrote {path}")
